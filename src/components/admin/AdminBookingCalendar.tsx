...
export function AdminBookingCalendar({ bookings }: AdminBookingCalendarProps) {
  ...
  function bookingsForDay(day: Date): BookingWithDetails[] {
    const key = format(day, 'yyyy-MM-dd')
    return bookings.filter(b => b.booking_date === key && b.status !== 'CANCELLED')
  }

  function dotColor(dayBookings: BookingWithDetails[]): string {
    ...
    <>
      <div className="card overflow-hidden">
        ...
        <div className="grid grid-cols-7 bg-bg-surface2 border-b border-border">
          {DAY_LABELS.map(l => (
            <div key={l} className="text-center text-[10px] font-semibold text-text-3
                ...
                {hasBookings && inMonth && (
                  <div className="flex items-center gap-0.5">
                    {dayBookings.slice(0, 3).map((_, idx) => (
                      <span key={idx}
                        className={`w-1.5 h-1.5 rounded-full ${dotColor(dayBookings)}`}
                      />
                    ))}
                    {dayBookings.length > 3 && (
                      <span className="text-[9px] text-text-3 leading-none">
                        +{dayBookings.length - 3}
                      </span>
                    )}
                  </div>
        ...
        {selectedBookings.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {selectedBookings.map(b => (
              <div key={b.id}
                className="bg-bg-surface2 rounded-lg p-3 flex items-start gap-3">
                <div className="bg-accent-soft border border-accent-mid rounded-md
                ...
                <div className="flex-shrink-0">{bookingStatusBadge(b.status)}</div>
              </div>
            ))}
