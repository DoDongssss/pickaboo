...
interface CourtFormModalProps {
  ...
  court:   Court | null
  onClose: () => void
  onSave:  (data: CourtFormData) => Promise<void>
}
...
function CourtFormInner({
  court, onClose, onSave
}: Omit<CourtFormModalProps, 'open'>) {
  const [form,    setForm]    = useState<CourtFormData>(
    () => court ? courtToForm(court) : EMPTY
  )
  ...
  function validate(): boolean {
    ...
    if (form.open_time >= form.close_time)
      e.close_time = 'Close time must be after open time.'
    setErrors(e)
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-medium text-text-2">
          Court Name <span className="text-status-error">*</span>
        </label>
        <input
          className={`input ${errors.name ? 'border-status-error' : ''}`}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <input
          className="input"
          placeholder="e.g. Indoor · Professional net · AC"
      ...
      {/* Address */}
      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-medium text-text-2">Address</label>
        <input
          className="input"
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-medium text-text-2">
          Price per Hour (₱) <span className="text-status-error">*</span>
        </label>
        <input
          className={`input ${errors.price_per_hour ? 'border-status-error' : ''}`}
          type="number" placeholder="300"
          value={form.price_per_hour || ''}
          onChange={e => set('price_per_hour', parseFloat(e.target.value))}
        />
        {errors.price_per_hour && (
          <p className="text-xs text-status-error">{errors.price_per_hour}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            className={`input ${errors.close_time ? 'border-status-error' : ''}`}
            type="time"
            value={form.close_time}
            onChange={e => set('close_time', e.target.value)}
          />
          {errors.close_time && (
            <p className="text-xs text-status-error">{errors.close_time}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between bg-bg-surface2 rounded-lg px-4 py-3">
        ...
        <label className="relative inline-flex items-center cursor-pointer">
          ...
          />
          <div className="w-10 h-6 bg-border-strong rounded-full peer
